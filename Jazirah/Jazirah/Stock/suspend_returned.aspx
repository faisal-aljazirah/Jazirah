<%@ Page Title="" Language="vb" AutoEventWireup="false" MasterPageFile="~/Stock/ST.Master" CodeBehind="suspend_returned.aspx.vb" Inherits="Jazirah.suspend_returned" %>
    ...
    <div class="col-xs-4">
        <div class="card p-1">
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6">
                        <button type="button" class="btn btn-sm btn-success full-width" onclick="javascript:showModal('viewSReturned', '{lngTransaction: 0, IsOut: false}', '#mdlAlpha')">Suspend Returned Items</button>
                    </div>
                    <div class="col-md-6">
                        <button type="button" class="btn btn-sm btn-success full-width" onclick="javascript:showModal('viewSReturned', '{lngTransaction: 0, IsOut: true}', '#mdlAlpha')">Resume Returned Items</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="col-xs-8">
        <div class="card p-1">
            <div class="card-body">
                <div class="row">
                    <div class="col-md-12">
                        <div class="col-md-3"><input type="text" class="form-control form-control-sm input-sm" id="txtVoucherSearch" placeholder="<%=plcItem %>" value="" /></div>
                        <div class="col-md-3"><select id="drpWarehouseSearch" class="form-control input-sm"><%=WarehouseList %></select></div>
                        ...
                        <div class="col-md-2"><button type="button" id="btnSearch" class="btn btn-sm btn-outline-orange full-width" onclick="javascript:fillSReturned();"><%=btnSearch%></button></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="col-xs-12">
        <div class="card">
            <div class="card-body collapse in p-1">
                <div class="table-responsive" id="tblReturned">
  ...
  <script type="text/javascript">
      ...
      function fillSExpired() {
          $('#tblExpired').html('<div class="bg-grey text-md-center text-bold-300 bg-lighten-4 pt-2 pb-2"><img src="../app-assets/images/icons/spinner.gif" /> <%=strWait %></div>');
                $.ajax({
                    type: 'POST',
                    url: '../Stock/ajax.aspx/fillSExpired',
                    data: '{}',
                    contentType: 'application/json; charset=utf-8',
                    dataType: 'json',
                    success: function (response) {
      ...
      function fillSReturned() {
          ...
          $('#tblReturned').html('<div class="bg-grey text-md-center text-bold-300 bg-lighten-4 pt-2 pb-2"><img src="../app-assets/images/icons/spinner.gif" /> <%=strWait %></div>');
                $.ajax({
                    type: 'POST',
                    url: '../Stock/ajax.aspx/fillSReturned',
      ...
      function approveItems(trans, modal) {
          $.ajax({
              type: 'POST',
              url: 'ajax.aspx/approveItems',
